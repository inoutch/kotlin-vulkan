...
data class Vertex(
    ...
    companion object {
        const val SIZE = (Vec3.SIZE + Vec3.SIZE + Vec2.SIZE)
...
class HelloTriangleApplication(private val delegate: Delegate) {
    ...
    private fun createSwapChain() {
        ...
        val indices = findQueueFamilies(physicalDevice)
        val queueFamilyIndices = if (indices.graphicsFamily == indices.presentFamily) {
            listOf(indices.graphicsFamily)
        } else {
            listOf(indices.graphicsFamily, indices.presentFamily)
        }
    ...
    private fun createFramebuffers() {
        swapChainFramebuffers = swapChainImageViews.map { swapChainImage ->
            val attachments = listOf(swapChainImage, depthImageView)
    ...
    private fun createTextureImage() {
        ...
        for (y in 0 until texHeight) {
            for (x in 0 until texWidth) {
                pixels[(y * texWidth + x) * 4 + 0] = ((x.toFloat() / texWidth.toFloat()) * 255).toByte()
                pixels[(y * texWidth + x) * 4 + 1] = ((y.toFloat() / texHeight.toFloat()) * 255).toByte()
                pixels[(y * texWidth + x) * 4 + 2] = -1
                pixels[(y * texWidth + x) * 4 + 3] = -1
            }
        }
    ...
    private fun createImageView(
            image: VkImage,
            format: VkFormat,
            aspectFlags: List<VkImageAspectFlagBits>
    ): VkImageView {
    ...
    private fun createVertexBuffer() {
        val bufferSize = (FLOAT_BYTE_SIZE * vertices.size).toLong()
    ...
    private fun createCommandBuffers() {
        ...
        for (i in commandBuffers.indices) {
            ...
            val renderPassInfo = VkRenderPassBeginInfo(
                    ...
                    listOf(
                            VkClearValue(VkClearColorValue(0.3f, 0.3f, 0.3f, 1.0f)),
    ...
    private fun updateUniformBuffer(currentImage: Int) {
        ...
        val ubo = UniformBufferObject(
                ...
                Mat4.createLookAt(Vec3(1.0f, 1.0f, 1.0f), Vec3(0.0f, 0.0f, 0.0f), Vec3(0.0f, 0.0f, 1.0f)),
