...
fun VkAttachmentDescription.copyToNative(native: vulkan_android.VkAttachmentDescription) {
...
fun VkAttachmentDescription.toNative(scope: MemScope) =
        scope.alloc<vulkan_android.VkAttachmentDescription>().also { copyToNative(it) }

@ExperimentalUnsignedTypes
fun List<VkAttachmentDescription>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_android.VkAttachmentDescription>(size).also {
