...
import vulkan_android.VK_STRUCTURE_TYPE_SUBMIT_INFO

@ExperimentalUnsignedTypes
fun VkSubmitInfo.copyToNative(
    native: vulkan_android.VkSubmitInfo,
    scope: MemScope
) {
...
fun VkSubmitInfo.toNative(scope: MemScope) =
        scope.alloc<vulkan_android.VkSubmitInfo>()
                .also { copyToNative(it, scope) }.ptr

@ExperimentalUnsignedTypes
fun List<VkSubmitInfo>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_android.VkSubmitInfo>(size)
