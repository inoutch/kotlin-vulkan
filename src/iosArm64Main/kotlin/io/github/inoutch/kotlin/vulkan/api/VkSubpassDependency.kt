...
fun VkSubpassDependency.copyToNative(native: vulkan_ios.VkSubpassDependency) {
...
fun VkSubpassDependency.toNative(scope: MemScope) =
        scope.alloc<vulkan_ios.VkSubpassDependency>().also { copyToNative(it) }

@ExperimentalUnsignedTypes
fun List<VkSubpassDependency>.toNative(scope: MemScope) =
        scope.allocArray<vulkan_ios.VkSubpassDependency>(size)
