...
import vulkan_ios.VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET

@ExperimentalUnsignedTypes
fun VkWriteDescriptorSet.copyToNative(
    native: vulkan_ios.VkWriteDescriptorSet,
    scope: MemScope
) {
...
fun VkWriteDescriptorSet.toNative(scope: MemScope) =
        scope.alloc<vulkan_ios.VkWriteDescriptorSet>()
                .also { copyToNative(it, scope) }.ptr

@ExperimentalUnsignedTypes
fun List<VkWriteDescriptorSet>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_ios.VkWriteDescriptorSet>(size)
