...
import vulkan_ios.VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO

@ExperimentalUnsignedTypes
fun VkDeviceQueueCreateInfo.copyToNative(
    native: vulkan_ios.VkDeviceQueueCreateInfo,
    scope: MemScope
) {
...
fun VkDeviceQueueCreateInfo.toNative(scope: MemScope) =
        scope.alloc<vulkan_ios.VkDeviceQueueCreateInfo>().also { copyToNative(it, scope) }.ptr

@ExperimentalUnsignedTypes
fun List<VkDeviceQueueCreateInfo>.toNative(scope: MemScope) =
        scope.allocArray<vulkan_ios.VkDeviceQueueCreateInfo>(size)
