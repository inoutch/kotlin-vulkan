...
import vulkan_ios.VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO

@ExperimentalUnsignedTypes
fun VkPipelineShaderStageCreateInfo.copyToNative(
    native: vulkan_ios.VkPipelineShaderStageCreateInfo,
    scope: MemScope
) {
...
fun List<VkPipelineShaderStageCreateInfo>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_ios.VkPipelineShaderStageCreateInfo>(size)
