...
import vulkan_ios.VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO

@ExperimentalUnsignedTypes
fun VkGraphicsPipelineCreateInfo.copyToNative(
    native: vulkan_ios.VkGraphicsPipelineCreateInfo,
    scope: MemScope
) {
...
fun VkGraphicsPipelineCreateInfo.toNative(scope: MemScope) =
        scope.alloc<vulkan_ios.VkGraphicsPipelineCreateInfo>().also { copyToNative(it, scope) }.ptr

@ExperimentalUnsignedTypes
fun List<VkGraphicsPipelineCreateInfo>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_ios.VkGraphicsPipelineCreateInfo>(size)
