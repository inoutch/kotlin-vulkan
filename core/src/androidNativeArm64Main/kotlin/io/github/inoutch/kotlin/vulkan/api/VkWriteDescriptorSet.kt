...
import vulkan_android.VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET

@ExperimentalUnsignedTypes
fun VkWriteDescriptorSet.copyToNative(
    native: vulkan_android.VkWriteDescriptorSet,
    scope: MemScope
) {
...
fun VkWriteDescriptorSet.toNative(scope: MemScope) =
        scope.alloc<vulkan_android.VkWriteDescriptorSet>()
                .also { copyToNative(it, scope) }.ptr

@ExperimentalUnsignedTypes
fun List<VkWriteDescriptorSet>.toNative(scope: MemScope) =
        if (isEmpty()) null else scope.allocArray<vulkan_android.VkWriteDescriptorSet>(size)
